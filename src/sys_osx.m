...
static void EzMaybeInstallAssetsDialog(NSString *selectedDirectory) {
	...
	NSArray *assetsContents = [[NSFileManager defaultManager] contentsOfDirectoryAtPath:assetsPath error:nil];
	if (assetsContents == nil || ![assetsContents count])
	{
		return;
	}

	NSString *assetNames = [assetsContents componentsJoinedByString:@"\n"];

	NSAlert *alert = [[NSAlert alloc] init];
	[alert setMessageText:@"Install extra content?"];
	[alert setInformativeText:[NSString stringWithFormat:@"Do you want to install the following assets?\n\n%@", assetNames]];
	[alert addButtonWithTitle:@"Yes"];
	[alert addButtonWithTitle:@"No"];
	if ([alert runModal] == NSAlertFirstButtonReturn)
	{
		// TODO: Install extra assets from the web
		// EzInstallAssets(selectedDirectory);
	}
}
...
static BOOL EzCheckExistingSettings(char *basedir, int length)
{
	...
	if (EzIsValidEzQuakeDirectory(resolvedPath))
	{
		...
		{
			EzMaybeInstallAssetsDialog(resolvedPath);
		}
