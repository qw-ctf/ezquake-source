...
)
{
	...
	// Skip if no vertex/fragment shader (e.g., compute-only program)
	if (!program->shaders[shadertype_vertex].text && !program->shaders[shadertype_fragment].text) {
		return false;
	}
...
static void GL_SetupProgramBindings(r_program_id program_id)
{
	...
	{
		GLuint idx = GL_GetUniformBlockIndex(prog, "GlobalState");
	...
	for (i = 0; i < (int)(sizeof(storage_bindings) / sizeof(storage_bindings[0])); i++) {
		GLuint idx = GL_GetUniformBlockIndex(prog, storage_bindings[i].name);
		if (idx != GL_INVALID_INDEX) {
...
qbool R_ProgramCompileWithInclude(r_program_id program_id, const char* included_definitions)
{
	...
	if (GL_CompileProgram(program)) {
		...
		GL_SetupProgramBindings(program_id);
		return true;
	}
	return false;
}
...
static void GL_BuildCoreDefinitions(void)
{
	...
	if (!R_UseModernOpenGL() && glConfig.core_profile) {
		strlcat(macro_definitions,
