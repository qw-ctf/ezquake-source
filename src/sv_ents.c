...
static void SV_WritePlayersToClient (client_t *client, client_frame_t *frame, byte *pvs, qbool disable_updates, sizebuf_t *msg)
{
	...
	for (j = 0; j < MAX_CLIENTS; j++)
	{
		...
		{
			// Without PEXT_TRANS there's no PF_EXTRA_PFS, so move back
			// PF_ONGROUND and PF_SOLID to their old offsets.
		...
		if (pflags & PF_TRANS_Z)
			MSG_WriteByte (msg, ent->xv.alpha ? bound(1, (byte)(ent->xv.alpha * 254.0f), 254) : 255);
#endif
	}
}
