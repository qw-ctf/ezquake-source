...
static void R_WarpSurfaceSubdividePolygon(int numverts, float *verts)
{
	...
	poly = (glpoly_t *)Hunk_AllocName(sizeof(glpoly_t) + (numverts - 4) * VERTEXSIZE * sizeof(float), "warpsurf");
...
void R_SkySurfacesBuildPolys(msurface_t *fa)
{
	...
	poly = Hunk_AllocName(sizeof(glpoly_t) + (numverts - 4) * VERTEXSIZE * sizeof(float), "skysurfs");
	poly->next = NULL;
	fa->polys = poly;
	poly->numverts = numverts;
	vert = verts[0];
